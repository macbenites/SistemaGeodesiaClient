...
const TableInventario = () => {
  ...
  doc.text (String(InventarioReport?.empresa),90,20); 
  doc.text (String(InventarioReport?.almacen),60,35); 
  doc.text ("Documento Inventario", 40, 37); 
  doc.text(String(InventarioReport?.fec_generado), 100, 47);
  doc.autoTable({ html: '#table-Inventario', margin: { top: 60 } });
