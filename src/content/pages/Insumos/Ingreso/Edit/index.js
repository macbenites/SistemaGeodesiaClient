import {
  ...
  const EditSupply = ({ modal, setModal }) => {
    ...
    const { updateSupplyIn, employe, msgUpdateIn } = useSelector(
      (state) => state.supplies
    );

    const dispatch = useDispatch(); 
    useEffect(() => {
      dispatch(fetchEmployee(updateSupplyIn.reg_ing_cab.cod_almacen));
    }, [dispatch]);
  
    return (
      <>
        <BasicModal modal={modal} setModal={setModal} message={msgUpdateIn} />
        <Container maxWidth="lg">
          ...
          >
            <Grid item xs={12}>
              <Card>
                <CardHeader title="Actualizar ingreso de insumo" />
                <Divider />
                <CardContent>
                  <Formik
                    initialValues={{
                      cod_reg_in: updateSupplyIn.reg_ing_cab.cod_reg_in,
                      cod_prov: updateSupplyIn.reg_ing_cab.cod_prov,
                      cod_trabajador: updateSupplyIn.reg_ing_cab.cod_trabajador,
                      cod_almacen: updateSupplyIn.reg_ing_cab.cod_almacen,
                      cod_t_transf: updateSupplyIn.reg_ing_cab.cod_t_transf,
                      cod_t_doc: updateSupplyIn.reg_ing_cab.cod_t_doc,
                      nro_doc: updateSupplyIn.reg_ing_cab.nro_doc,
                      fec_doc: updateSupplyIn.reg_ing_cab.fec_doc,
                      articles: updateSupplyIn.reg_ing_det.map((article)=> {
                        return{
                            cod_art: article.cod_art,
                            prec_unit: article.prec_unit,
                            cant_art: article.cant_art,
                            obs_ing: article.obs_ing
                        };
                      })
                    }}
                    validationSchema={validationSupplies}
                    onSubmit={async (values, { resetForm }) => {
                      dispatch(saveUpdateIngreso(values)).then(() => {
                        setModal(false);
                        resetForm();
                      });
                    }}
                      ...
                      <Form autoComplete="off">
                        <Grid container spacing={2}>
                          <Grid item xs={12} md={4}>
                            ...
                            >
                              {updateSupplyIn.proveedor?.map((supply) => (
                                <MenuItem
                                  key={supply.cod_prov}
                          ...
                          <Grid item xs={12} md={5}>
                            <TextField
                              ...
                              onChange={async (e) => {
                                ...
                                setFieldValue('cod_trabajador', employe);
                              }}
                            //   error={
                            ...
                            >
                              {updateSupplyIn.almacen?.map((supply) => (
                                <MenuItem
                          ...
                          <Grid item xs={12} md={4}>
                            ...
                            >
                              {updateSupplyIn.tipo_transf?.map((supply) => (
                                <MenuItem
                          ...
                          <Grid item xs={12} md={4}>
                            ...
                            >
                              {updateSupplyIn.tipo_doc_reg?.map((supply) => (
                                <MenuItem
